...
BACKUP_PATH = DOCX_PATH.with_name(DOCX_PATH.stem + "_backup_nom_entreprise_strict.docx")

# Normalize apostrophes and whitespace for robust matching.
def norm(s: str) -> str:
    s = s.replace("’", "'").replace("`", "'")
    s = re.sub(r"\s+", " ", s)
    return s.strip().lower()

TARGET = norm("NOM DE L’ENTREPRISE")
...
for name, data in list(files.items()):
    ...
    # 1) Remove VML blocks (<v:rect>/<v:shape>) if their full content contains target.
    ...
    # 2) Remove Word paragraphs if merged <w:t> content matches target.
    ...
    # 3) Remove DrawingML paragraphs if merged <a:t> content matches target.
    ...
    # 4) Last resort: remove exact raw text occurrences that may remain.
    before = text
    text = text.replace("NOM DE L’ENTREPRISE", "")
    text = text.replace("NOM DE L'ENTREPRISE", "")
    if text != before:
